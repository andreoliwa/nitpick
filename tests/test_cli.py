...
def test_config_file_already_has_tool_nitpick_section(tmp_path, config_file):
    ...
    project.cli_init(f"The config file {config_file} already has a [{CONFIG_TOOL_NITPICK_KEY}] section.", exit_code=1)
...
def test_create_basic_dot_nitpick_toml(tmp_path):
    ...
    project.cli_init(
        f"A [{CONFIG_TOOL_NITPICK_KEY}] section was created in the config file: {DOT_NITPICK_TOML}"
...
def test_init_empty_pyproject_toml(tmp_path):
    ...
    project.pyproject_toml("").cli_init(
        f"A [{CONFIG_TOOL_NITPICK_KEY}] section was created in the config file: {PYTHON_PYPROJECT_TOML}"
    ).assert_file_contents(
...
def test_add_tool_nitpick_section_to_pyproject_toml(tmp_path, styles):
    ...
    project.cli_init(
        f"A [{CONFIG_TOOL_NITPICK_KEY}] section was created in the config file: {PYTHON_PYPROJECT_TOML}", *styles
    ).assert_file_contents(
        PYTHON_PYPROJECT_TOML,
        f"""
        [tool.black]
