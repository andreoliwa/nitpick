...
def init(context, force: bool, suggest: bool, style_urls: list[str]) -> None:
    ...
    if tool_nitpick_table and not force:
        click.secho(
            f"The config file {path.name!r} already has a [{CONFIG_TOOL_NITPICK_KEY}] section."
    ...
    if style_array is None:
        # TODO(AA): test when there is a [tool.nitpick] section but no [tool.nitpick.style] key
    ...
    if suggest:
        ...
        tomlkit_ext.update_comment_before(
            ...
            New styles added to the Nitpick library are automatically suggested here.
